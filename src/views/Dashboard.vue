<template>
  <DefaultLayout>
    <div class="flex flex-col h-full">
      <div class="flex-1">
        <div class="md:grid gap-6 md:grid-cols-2">
          <div class="p-6 bg-white dark:bg-graydark rounded-lg shadow-md mb-4">
            ...
            <div class="relative overflow-hidden">
              <div class="flex transition-transform duration-500 ease-in-out" :style="{ transform: `translateX(-${currentSlide * 100}%)` }">
                  ...
                  <div class="w-full grid grid-cols-4 items-center">
                    <div class="col-span-2 w-auto">
                      <p v-if="totalesInfo" class="text-lg text-gray-500">
                        Total licencias: <b>{{ totalesInfo.TotalesLicencias[0].total + totalesInfo.TotalesLicencias[1].total }}</b>
                      </p>
                      <div class="flex flex-wrap gap-2">
                        <p v-if="totalesInfo" class="text-base text-gray-800 mt-2">
                        <span class="dark:bg-black py-2 px-3 rounded-md shadow-md">{{ totalesInfo.TotalesLicencias[0].total }}</span> Básica
                      </p>
                        <p v-if="totalesInfo" class="text-base text-gray-800 mt-2">
                          <span class="dark:bg-black py-2 px-3 rounded-md shadow-md">{{ totalesInfo.TotalesLicencias[1].total }}</span> Avanzada
                        </p>
                      </div>

                    </div>
                    <div class="col-span-2 flex justify-center w-auto">
                      <img class="w-auto h-44 object-contain dark:hidden" src="../assets/general/img1.png" alt="licencias">
                      <img class="w-auto h-44 object-contain dark:block hidden" src="../assets/general/img1Dark.png" alt="licencias">
                    </div>
                  </div>
                </div>
                ...
                <div class="w-full px-10 flex-shrink-0">
                  ...
                  <div class="w-full grid grid-cols-4 items-center">
                    <div class="col-span-2 w-auto">
                      <p v-if="totalesInfo" class="text-lg text-gray-500">
                        Total Usuarios: <b>{{ totalesInfo.Estudiantes + totalesInfo.Docentes + totalesInfo.Clientes }}</b>
                      </p>
                      <div class="flex flex-wrap gap-2">
                        <p v-if="totalesInfo" class="text-base text-gray-800 mt-2">
                          <span class="dark:bg-black py-2 px-3 rounded-md shadow-md">{{ totalesInfo.Clientes }}</span> Clientes
                        </p>
                        <p v-if="totalesInfo" class="text-base text-gray-800 mt-2">
                        <span class="dark:bg-black py-2 px-3 rounded-md shadow-md">{{ totalesInfo.Docentes }}</span> Docentes
                      </p>
                        <p v-if="totalesInfo" class="text-base text-gray-800 mt-2">
                          <span class="dark:bg-black py-2 px-3 rounded-md shadow-md">{{ totalesInfo.Estudiantes }}</span> Estudiantes
                        </p>
                      </div>
              ...
              <!-- Controles -->
              <button @click="prevSlide" class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full hover:bg-opacity-75">
                ❮
              </button>
              <button @click="nextSlide" class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white p-2 rounded-full hover:bg-opacity-75">
                ❯
              </button>
            </div>
    ...
    <tbody v-if="totalesInfo">
      <tr v-for="(item, index) in  totalesInfo.instituciones" :key="index">
                        <td class="py-3 pl-3" v-tooltip.left="item.nombre +' '+ item.apellidos">
                          ...
                          <Avatar v-if="item.logo != null" class="w-16 h-16 dark:bg-graydark shadow-md p-1" :image="getInitials(item.nombre)" shape="circle" />
                          <Avatar v-else class="w-16 h-16 flex items-center justify-center shadow-md dark:bg-graydark p-1" :label="getInitials(item.nombre)" shape="circle" />
                        </td>
