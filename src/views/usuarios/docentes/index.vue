...
const checkCargaMasiva = async () => {
  try {
    const res = await axios.get('/api/notificacion-carga-docentes')
    if (res.data.completado) {
      notificacion.value = res.data.mensaje
      if (polling) clearInterval(polling)
      localStorage.removeItem('cargaMasivaEnviada1') // Limpiar el estado de carga masiva
      swal.fire({
        icon: 'success',
...
const iniciarPolling = () => {
  ...
  if (localStorage.getItem('cargaMasivaEnviada1') === 'true') {
