<template>
  <div class="w-full">
    <div class="w-full flex justify-start">
      <breadcrumb-default
        pageTitle="Añadir docentes masivo"
        pageSubtitle="Docentes"
        path="docentes"
      ></breadcrumb-default>
    </div>
  </div>
  <div class="mt-4 w-full bg-white dark:bg-boxdark p-2 rounded-md shadow-md">
    <h2 class="text-lg font-bold mb-4">Cargue masivo Docentes</h2>
    <p class="sm:text-lg">
      <b>Nota:</b> Para subir masivamente la lista de docentes, debe descargar la plantilla, dando
    ...
    <div
      ...
      class="dark:bg-graydark bg-white text-gray-500 p-4 rounded-md shadow-md flex flex-wrap items-center h-full gap-4 mt-4"
    >
...
const descargarPlantilla = async () => {
  const response = await axios.get('/api/plantilla-docentes', { responseType: 'blob' })
  console.log(response)

  const url = window.URL.createObjectURL(new Blob([response.data]))
  const link = document.createElement('a')
  link.href = url
  link.setAttribute('download', 'plantilla_docentes.xls')
...
const cargarArchivo = async () => {
  ...
  formData.append('docentes', archivo.value);
