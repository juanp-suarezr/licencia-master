<template>
  <div class="w-full flex justify-start">
    <breadcrumb-default pageTitle="Editar Rol" pageSubtitle="Roles" path="roles"></breadcrumb-default>
  </div>
        ...
        <div class="mb-4">
          ...
          <div class="relative">
            <button @click="toggleDropdown" type="button"
              ...
              <span>{{ selectedPermissions.length ? selectedPermissions.length+' permisos seleccionados' : 'Selecciona los permisos' }}</span>
...
const submit = async () => {
  ...
  try {
    await axios.patch(`/api/roles/${route.params.id}`, role, {
      headers: {
    ...
    swal.fire({
      icon: 'error',
      title: 'Error en la actualización del rol',
...
onMounted(async () => {
  try {
    const response = await axios.get(`/api/roles/${route.params.id}/edit`);
    console.log(response);

    if (response.data.success) {
      const roleData: Role = response.data.data.role;
      const permissionsData: Permission[] = response.data.data.permission;
      const rolePermissionsData: number[] = Object.values(response.data.data.rolePermissions);

      role.name = roleData.name;
      role.permisos = rolePermissionsData;
      selectedPermissions.value = rolePermissionsData;
