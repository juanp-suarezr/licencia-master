<template>
  <div class="w-full flex justify-start">
    <breadcrumb-default
      pageTitle="Objetos registraduría tienda"
      pageSubtitle="Parámetros"
      path="parametros"
    ></breadcrumb-default>
  </div>
  <div class="w-full flex flex-wrap items-center">
    <!-- total parametros -->
    <div
      class="w-full dark:bg-graydark bg-white text-gray-500 p-4 rounded-md shadow-md flex items-center h-full"
    >
      <!-- info -->
      <div class="w-full">
  ...
  <div class="w-full mt-6 bg-white dark:bg-boxdark p-2 rounded-md shadow-md">
    <div class="flex flex-wrap justify-between items-center pe-2">
      <h2 class="mt-2 px-4 text-base">Objetos tienda</h2>
      <button
        @click="openAddModal"
      ...
      <table class="table-auto w-full bg-white dark:bg-boxdark text-sm">
        <thead>
          <tr class="bg-gray-2 text-left dark:bg-meta-4">
            ...
            <th class="py-2 px-2 font-medium text-black dark:text-white text-left">PRECIO</th>
            <th class="py-2 px-2 font-medium text-black dark:text-white text-left">DEFAULT</th>
        ...
        <tbody v-if="parametro.length != 0">
          <tr v-for="(item, index) in parametro" :key="index" class="text-sm">
            ...
            <td class="py-3 px-3 whitespace-nowrap text-left">
              <h5 class="font-medium text-graydark dark:text-gray text-xs">{{ item.precio }}</h5>
            </td>
            <td class="py-3 px-3 whitespace-nowrap text-left">
              <h5 class="font-medium text-graydark dark:text-gray text-xs">
                {{ item.is_default ? 'Gratis por default' : 'No' }}
              </h5>
            </td>
            <td class="py-3 px-3 whitespace-normal break-words text-left">
              <h5 class="font-medium text-graydark dark:text-gray text-xs">
            ...
            <td class="py-3 px-2 gap-2 whitespace-nowrap text-left flex flex-wrap items-center">
              <button
                v-tooltip.left="'Bloquear'"
                @click="openEditModal(item.id)"
                class="p-2 hover:scale-105 bg-primary dark:text-white rounded-md shadow-md flex flex-wrap items-center gap-2"
              >
              ...
              <button
                ...
                @click="DesbloquearParametro(item.id)"
  ...
  <Modal :isOpen="addModalOpen" @close="closeAddModal">
    <div class="p-4 bg-white dark:bg-boxdark rounded-md shadow-md">
      ...
      <div class="mt-4 flex gap-2 items-center">
        <input id="is_default" v-model="newParametro.is_default" type="checkbox" class="" />
        <label for="is_default" class="block text-sm font-medium text-gray-700 mb-2"
          >guardado por default para el jugador</label
        >
      </div>
      <div class="flex justify-end mt-4 gap-2">
        <button
          @click="submitAddParametro"
  ...
  <Modal :isOpen="editModalOpen" @close="closeEditModal">
    <div class="p-4 bg-white dark:bg-boxdark rounded-md shadow-md">
      <h3 class="text-lg font-bold">Editar parámetro</h3>
      <!-- Nombre -->
      <div class="mt-4">
        <label for="nombre" class="block text-sm font-medium text-gray-700 mb-2">Nombre</label>
        <input
          id="nombre"
          v-model="editarParametro.nombre"
          type="text"
          class="p-2 rounded-md bg-gray dark:bg-graydark placeholder:text-xs shadow-md w-full"
        />
      </div>
      ...
      <div class="mt-4">
        ...
        <input
          id="codigo"
          v-model="editarParametro.codigo"
          type="text"
          class="p-2 rounded-md bg-gray dark:bg-graydark placeholder:text-xs shadow-md w-full"
        />
      </div>
      ...
      <div class="mt-4">
        ...
        <input
          id="precio"
          v-model="editarParametro.precio"
          type="text"
          class="p-2 rounded-md bg-gray dark:bg-graydark placeholder:text-xs shadow-md w-full"
        />
      </div>
      <!-- default -->
      <div class="mt-4 flex gap-2 items-center">
        <input id="is_default" v-model="editarParametro.is_default" type="checkbox" class="" />
        <label for="is_default" class="block text-sm font-medium text-gray-700 mb-2"
          >guardado por default para el jugador</label
        >
      </div>
...
const closeAddModal = () => {
  ...
  newParametro.value = { nombre: '', codigo: '', precio: '', is_default: false }
  validationMessage.value = ''
}
// Función para abrir modal de editar
const openEditModal = (id: number) => {
  const parametroToEdit = parametro.value.find((item) => item.id === id)
  if (parametroToEdit) {
    editarParametro.value = { ...parametroToEdit }
...
const closeEditModal = () => {
  ...
  editarParametro.value = { nombre: '', codigo: '', precio: '', is_default: false }
  validationMessage.value = ''
}

const fetchParametro = async () => {
  const response = await axios.get('/api/parametros/objetos-tienda', {
    params: {
      search: search.value,
      estado: estado.value,
...
const submitEliminar = async () => {
  try {
    const response = await axios.delete(`/api/parametros/objetos-tienda/${idEliminar.value}`)
...
const DesbloquearParametro = async (idDesbloq) => {
  try {
    const response = await axios.put(`/api/parametros/objetos-tienda/unlock/${idDesbloq}`)
...
const submitAddParametro = async () => {
  ...
  const existingParametro = parametro.value.find(
    (item) => item.codigo === newParametro.value.codigo
  )
  ...
  try {
    const response = await axios.post('/api/parametros/objetos-tienda', newParametro.value, {
      headers: {
...
const submitEditParametro = async () => {
  // Validar que el código no se repita
  const existingParametro = parametro.value.find(
    (item) => item.codigo === editarParametro.value.codigo && item.id !== editarParametro.value.id
  )
  if (existingParametro) {
    validationMessage.value = 'El código ya existe.'
    return
  ...
  try {
    const response = await axios.put(
      `/api/parametros/objetos-tienda/${editarParametro.value.id}`,
      editarParametro.value,
      {
        headers: {
          'Content-Type': 'application/json',
          accept: 'application/json',
    ...
    swal.fire({
      icon: 'success',
      title: 'Parámetro editado con éxito',
      html: `
        <p>${response.data.message}</p>
        <p><strong>Parámetro:</strong> ${response.data.data.nombre}</p>`,
      customClass: {
    ...
    swal.fire({
      icon: 'error',
      title: 'Error al editar el parámetro',
      html: `
        <p>Ocurrió un error inesperado, vuelva a intentar más tarde</p>`,
      customClass: {
        popup: 'dark:bg-slate-900 dark:text-gray bg-white text-graydark',
