<template>
    <div class="w-full flex justify-start">
        <breadcrumb-default pageTitle="Area de juegos" pageSubtitle="Parámetros" path="parametros"></breadcrumb-default>
    </div>
    ...
    <div class="w-full mt-6 bg-white dark:bg-boxdark p-2 rounded-md shadow-md">
        <div class="flex flex-wrap justify-between items-center pe-2">
            <h2 class="mt-2 px-4 text-base">Area de juegos</h2>
...
const fetchParametro = async () => {
    ...
    const response = await axios.get('/api/parametros/area-juegos', {
        params: {
            search: search.value,
            estado: estado.value,
...
const submitEliminar = async () => {
    try {
        const response = await axios.delete(`/api/parametros/area-juegos/${idEliminar.value}`);
...
const DesbloquearParametro = async (idDesbloq) => {
    try {
        const response = await axios.put(`/api/parametros/area-juegos/unlock/${idDesbloq}`);
...
const submitAddParametro = async () => {
    ...
    try {
        const response = await axios.post('/api/parametros/area-juegos', newParametro.value, {
            headers: {
