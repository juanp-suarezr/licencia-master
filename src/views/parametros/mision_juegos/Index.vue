<template>
    <div class="w-full flex justify-start">
        <breadcrumb-default pageTitle="Misión juegos" pageSubtitle="Parámetros" path="parametros"></breadcrumb-default>
    </div>
    ...
    <div class="w-full mt-6 bg-white dark:bg-boxdark p-2 rounded-md shadow-md">
        <div class="flex flex-wrap justify-between items-center pe-2">
            <h2 class="mt-2 px-4 text-base">Misión juegos</h2>
...
const fetchParametro = async () => {
    ...
    const response = await axios.get('/api/parametros/mision-juegos', {
        params: {
            search: search.value,
            estado: estado.value,
...
const submitEliminar = async () => {
    try {
        const response = await axios.delete(`/api/parametros/mision-juegos/${idEliminar.value}`);
...
const DesbloquearParametro = async (idDesbloq) => {
    try {
        const response = await axios.put(`/api/parametros/mision-juegos/unlock/${idDesbloq}`);
...
const submitAddParametro = async () => {
    ...
    try {
        const response = await axios.post('/api/parametros/mision-juegos', newParametro.value, {
            headers: {
