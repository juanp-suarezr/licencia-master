<template>
  ...
  <div class="w-full flex flex-wrap items-center">
    ...
    >
      <!-- info -->
      <div class="w-full">
        <h3 class="text-sm font-bold">INFO APK</h3>
      </div>
      <!-- icono -->
      <div class="w-auto flex items-center justify-end">
        <span class="bg-primary/20 shadow-lg p-2 rounded-md">
          <DeviceTabletIcon class="h-6 w-6 text-primary/60" />
        </span>
      </div>
    </div>
  </div>
  ...
  <div class="w-full mt-6 bg-white dark:bg-boxdark p-2 rounded-md shadow-md">
    <div class="flex flex-wrap justify-between items-center pe-2">
      <h2 class="mt-2 px-4 text-base">Información APK</h2>
      <button
        @click="openAddModal"
      ...
      <table class="table-auto w-full bg-white dark:bg-boxdark text-sm">
        <thead>
          <tr class="bg-gray-2 text-left dark:bg-meta-4">
            ...
            <th class="py-2 px-2 font-medium text-black dark:text-white text-left">DETALLLE</th>
            <th class="py-2 px-2 font-medium text-black dark:text-white text-left">VALOR</th>
        ...
        <tbody v-if="parametro.length != 0">
          <tr v-for="(item, index) in parametro" :key="index" class="text-sm">
            ...
            <td class="py-3 px-3 whitespace-normal break-words text-left">
              <h5 class="font-medium text-graydark dark:text-gray text-xs">{{ item.detalle || '' }}</h5>
            </td>
            <td class="py-3 px-3 whitespace-nowrap text-left">
              <h5 class="font-medium text-graydark dark:text-gray text-xs">{{ item.valor || ''}}</h5>
            </td>
  ...
  <Modal :isOpen="addModalOpen" @close="closeAddModal">
    <div class="p-4 bg-white dark:bg-boxdark rounded-md shadow-md">
      <h3 class="text-lg font-bold">Añadir nueva información de la APK</h3>
      <!-- Detalle -->
      <div class="mt-4">
        <label for="detalle" class="block text-sm font-medium text-gray-700 mb-2">Detalle</label>
        <input
          id="detalle"
          v-model="newParametro.detalle"
          type="text"
  ...
  <Modal :isOpen="editModalOpen" @close="closeEditModal">
    <div class="p-4 bg-white dark:bg-boxdark rounded-md shadow-md">
      ...
      <div class="mt-4">
        ...
        <input
          id="detalle"
          v-model="editarParametro.detalle"
          type="text"
          class="p-2 rounded-md bg-gray dark:bg-graydark placeholder:text-xs shadow-md w-full"
        />
      </div>
...
const closeAddModal = () => {
  ...
  newParametro.value = { detalle: '', valor: '', estado: 1 }
...
const closeEditModal = () => {
  ...
  editarParametro.value = { id: 0, detalle: '', valor: '', }
  validationMessage.value = ''
}

const fetchParametro = async () => {
  const response = await axios.get('/api/parametros/info-apk', {
    params: {
      estado: estado.value,
...
const submitEliminar = async () => {
  try {
    const response = await axios.delete(`/api/parametros/info-apk/${idEliminar.value}`)
    ...
    swal.fire({
      ...
      html: `
        ...
        <p><strong>Parámetro:</strong> ${response.data.data.detalle} - ${response.data.data.valor}</p>`,
      customClass: {
...
const DesbloquearParametro = async (idDesbloq:number) => {
  try {
    const response = await axios.put(`/api/parametros/info-apk/unlock/${idDesbloq}`)
...
const submitAddParametro = async () => {
  try {
    const response = await axios.post('/api/parametros/info-apk', newParametro.value, {
      headers: {
    ...
    swal.fire({
      ...
      html: `
        <p>${response.data.message}</p>
        <p><strong>Parámetro:</strong> ${response.data.data.detalle}</p>`,
      customClass: {
        popup: 'dark:bg-slate-900 dark:text-gray bg-white text-graydark',
        title: 'dark:text-gray text-graydark',
        confirmButton:
...
const submitEditParametro = async () => {
  try {
    const response = await axios.put(
      `/api/parametros/info-apk/${editarParametro.value.id}`,
