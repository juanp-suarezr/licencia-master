<template>
    <div class="w-full flex justify-start">
        <breadcrumb-default pageTitle="Sub-ambiente de juegos" pageSubtitle="Parámetros"
    ...
    <div class="w-full mt-6 bg-white dark:bg-boxdark p-2 rounded-md shadow-md">
        <div class="flex flex-wrap justify-between items-center pe-2">
            <h2 class="mt-2 px-4 text-base">Sub-ambiente de juegos</h2>
        ...
        <div class="mt-4 px-4 w-full flex flex-wrap justify-between items-center">
            ...
            <div class="flex flex-wrap gap-4">
                <div class="flex flex-col text-sm">
                    ...
                    <select id="estado" v-model="estado" @change="resetPagination"
                ...
                <div class="flex flex-col text-sm">
                    <label for="locaciones" class="block text-xs font-medium text-gray-700 mb-1">Locaciones</label>
                    <select id="locaciones" v-model="selectedLocacion" @change="resetPagination"
                        class="p-2 rounded-md bg-gray dark:bg-graydark justify-end shadow-md">
                        <option value="">Todos</option>
                        <option v-for="loc in locaciones" :key="loc.id" :value="loc.id">{{loc.nombre}}</option>
                    </select>
                </div>

                <div class="flex flex-col text-sm">
                    <label for="perPage" class="block text-xs font-medium text-gray-700 mb-1">Paginado</label>
                    <select id="perPage" v-model="perPage" @change="resetPagination"
                        class="p-2 rounded-md bg-gray dark:bg-graydark justify-end shadow-md">
    ...
    <Modal :isOpen="addModalOpen" @close="closeAddModal">
        <div class="p-4 bg-white dark:bg-boxdark rounded-md shadow-md">
            ...
            <div class="flex flex-col mt-4">
                    <label for="locaciones" class="block text-sm font-medium text-gray-700 mb-2">Seleccionar Locación</label>
                    <select id="locaciones" v-model="newParametro.id_padre"
                        class="p-2 rounded-md bg-gray dark:bg-graydark justify-end shadow-md">
                        <option v-for="loc in locaciones" :key="loc.id" :value="loc.id">{{loc.nombre}}</option>
                    </select>
                </div>
...
const closeAddModal = () => {
    ...
    newParametro.value = { nombre: '', codigo: '', id_padre: '', };
    validationMessage.value = '';
};

const fetchParametro = async () => {
    const response = await axios.get('/api/parametros/subambiente-juegos', {
        params: {
            search: search.value,
            estado: estado.value,
            locacion: selectedLocacion.value,
    ...
    parametro.value = response.data.data.subambiente_juegos.data;
    totalParametros.value = response.data.data.subambiente_juegos.total;
    locaciones.value = response.data.data.locacion;
    totalPages.value = response.data.data.subambiente_juegos.last_page;
};
...
const submitEliminar = async () => {
    try {
        const response = await axios.delete(`/api/parametros/subambiente-juegos/${idEliminar.value}`);
...
const DesbloquearParametro = async (idDesbloq) => {
    try {
        const response = await axios.put(`/api/parametros/subambiente-juegos/unlock/${idDesbloq}`);
...
const submitAddParametro = async () => {
    ...
    try {
        const response = await axios.post('/api/parametros/subambiente-juegos', newParametro.value, {
            headers: {
