<template>
    <div class="w-full flex justify-start">
        <breadcrumb-default pageTitle="Locación de juegos" pageSubtitle="Parámetros"
    ...
    <div class="w-full mt-6 bg-white dark:bg-boxdark p-2 rounded-md shadow-md">
        <div class="flex flex-wrap justify-between items-center pe-2">
            <h2 class="mt-2 px-4 text-base">Locación de juegos</h2>
...
const fetchParametro = async () => {
    ...
    const response = await axios.get('/api/parametros/locacion-juegos', {
        params: {
            search: search.value,
            estado: estado.value,
...
const submitEliminar = async () => {
    try {
        const response = await axios.delete(`/api/parametros/locacion-juegos/${idEliminar.value}`);
...
const DesbloquearParametro = async (idDesbloq) => {
    try {
        const response = await axios.put(`/api/parametros/locacion-juegos/unlock/${idDesbloq}`);
...
const submitAddParametro = async () => {
    ...
    try {
        const response = await axios.post('/api/parametros/locacion-juegos', newParametro.value, {
            headers: {
